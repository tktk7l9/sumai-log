...
- **記録の分析**（`/analysis`、ヘッダのアイコン）: 見学・動画メモ・業者・予定・コメントをアプリの中で数える
  （外部に送らない）。件数と期間・月ごとの推移・業者ごとの接点・よく出る用語（用語集）・よかった点／
  気になる点によく出る言葉・未完了の「次にやること」・書きかけの記録・タグ・チャンネル・誰が行ったか
- **区画シミュレーター**（`/site`、ヘッダのアイコン）: 大きな土地の一部を敷地にするとき、道路側（手前）と奥の
  どちらに区画を取るかを図の上で指で動かして比べる。奥に取ると道路までの通路（路地状部分）を
  敷地に含め、接道・残りの土地の接道・建ぺい率/容積率（前面道路の幅員×0.4 も）・境界からの離れ・
  準防火地域の延焼ライン（隣地 3m・道路中心線 3m）・筆界をまたぐか・南側の空き（南が道路なら幅員も数える）・
  冬至の南の窓の日照（隣地の建物の高さと土地の実際の向き・緯度から、8〜16 時を 10 分刻み）を判定する。
  隣地・周りの建物（位置・大きさ・高さ）を図に描き、季節と時刻を選ぶと建物の影を重ねる。
  建物は平屋（既定）と 2 階建て（総 2 階として建築面積＝延床÷2・2 階の延焼ライン 5m）を切り替えられる。
  「3D」に切り替えると three.js で土地・隣地・建物を立体で見られる（指で回す・ピンチで寄る。
  影は選んだ季節・時刻の太陽から落とす。three.js はこのときだけ読み込み、Worker には入れない）。手前に
