...
| 記録の分析 | `/analysis` | 見学・動画メモ・業者・予定・コメントをアプリの中で集計（外部送信なし）。件数と期間、月ごとの推移、業者ごとの接点、よく出る用語・言葉、未完了の次にやること、書きかけの記録、タグ・チャンネル、誰が行ったか。ヘッダから開く。2026-09-23 追加 |
| 区画シミュレーター | `/site` | 大きな土地（長方形で近似）の一部を敷地として切り出し、区画と建物（平屋・2 階建て）を図の上で動かして面積・接道（路地状部分）・残りの土地の接道・建ぺい率/容積率（前面道路の幅員制限を含む）・境界からの離れ・準防火地域の延焼ライン・筆界・南側の空き・冬至の日照（隣地の建物の影）を確かめる。隣地・周りの建物を描き、季節と時刻で影を重ねる。3D 表示（three.js・遅延読み込み）に切り替えられる。区画の奥に土地が残るときは、残りの土地（月極駐車場など）への通路を土地の端に空けられる（区画はその帯を避ける）。ヘッダから開く。2026-09-23 追加 |
