...
| 区画シミュレーター | `/site` | 大きな土地（長方形で近似）の一部を敷地として切り出し、区画と平屋を図の上で動かして面積・接道（路地状部分）・残りの土地の接道・建ぺい率/容積率（前面道路の幅員制限を含む）・境界からの離れ・準防火地域の延焼ライン・筆界・南側の空き・冬至の日照（隣地の建物の影）を確かめる。隣地・周りの建物を描き、季節と時刻で影を重ねる。区画の奥に土地が残るときは、残りの土地（月極駐車場など）への通路を土地の端に空けられる（区画はその帯を避ける）。ヘッダから開く。2026-09-23 追加 |
