...
| 区画シミュレーター | `/site` | 大きな土地（長方形で近似）の一部を敷地として切り出し、区画と平屋を図の上で動かして面積・接道（路地状部分）・残りの土地の接道・建ぺい率/容積率・外壁後退・南側の空きを確かめる。区画の奥に土地が残るときは、残りの土地（月極駐車場など）への通路を土地の端に空けられる（区画はその帯を避ける）。ヘッダから開く。2026-09-23 追加 |
